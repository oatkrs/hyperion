# PROJECT HYPERION: THE OBSIDIAN MONOLITH
...
When you close Hyperion, we don't just `free()` memory. We use `secure_wipe()` to overwrite all sensitive RAM (keys, passwords, the memory grid) with zeros using volatile pointers to prevent compiler optimization.
...
```powershell
# Compile the Vault
gcc hyperion_v5.c -o hyperion.exe -O3

# Compile the Chrome Importer (Bridge)
gcc hyperion_bridge.c -o bridge.exe -O3
...
```bash
# Compile the Vault
gcc hyperion_v5.c -o hyperion -O3

# Compile the Chrome Importer (Bridge)
gcc hyperion_bridge.c -o bridge -O3
...
```bash
./hyperion my_secret_usb.vault

```

---

## THE BRIDGE (Import from Chrome(ium))
